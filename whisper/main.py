...
qa_prompt = """
You are an AI assistant role-playing as an expert panel in a generic presentation session. The presenter has just finished sharing their innovative proposal, aimed at solving a notable issue within their field. Your role is to engage with the presenter by posing insightful questions that probe into areas not fully explored in their presentation or aspects identified as needing improvement. Based on the content of their presentation, while avoiding topics already comprehensively addressed, generate three detailed questions aimed at enhancing the presenter's insight and response to potential critiques:

Depth and Relevance Question: If the presentation lacked depth in explaining how the proposal fits into broader context or market trends, formulate a question that examines the proposal's relevance and potential for impact. Focus on their approach to integrating with current standards and anticipating future developments in the field.

Innovation and Differentiation Question: If the presentation's innovation or unique value proposition was underdeveloped, create a question that inquires deeper into the unique aspects of their proposal. Investigate how their approach differentiates from existing solutions and its adaptability to upcoming changes or advancements.

Strategic and Operational Question: Should the presentation skim over strategic planning or operational viability, draft a question regarding their strategic roadmap. This should concentrate on execution plans, growth forecasts, critical upcoming milestones, and how they intend to measure and achieve success.

The aim is to foster a challenging and constructive Q&A session that encourages the presenter to thoroughly articulate and defend their proposal, particularly emphasizing the strengthening of its less convincing sections. This activity is intended to assist the presenter in clarifying and affirming the critical elements of their proposal that are essential for gaining the support and recognition of peers or stakeholders.

Your exact response will be spoken, so Do not include the question class (Depth and Relevance Question, Innovation and Differentiation Question, Strategic and Operational Question) or numbers in your response. 

Here is the presentation transcript:
{video_text}
"""

questions = gpt(prompt=qa_prompt.format(video_text=video_text))

questions = questions.strip('Depth and Relevance Question:')
questions = questions.strip('Innovation and Differentiation Question:')
questions = questions.strip('Strategic and Operational Question:')
...
# Save to file
response.stream_to_file(f'{temp}_questions.mp3')
print('done')